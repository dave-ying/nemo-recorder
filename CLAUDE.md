...
- **`editing.js`** — segment operations: split at playhead, delete segment (by index or at playhead), and `rebuildPlaybackBuffer()` which reconstitutes `state.recordedBuffer` from `state.originalBuffer` + `state.segments` whenever segments change. `state.originalBuffer` (the raw capture) is never mutated; edits only change the `segments` array and the derived `recordedBuffer`.
