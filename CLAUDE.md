...
- **`waveform.js`** — canvas rendering for the playback waveform: segment "cards" with gaps, peak computation from the recorded buffer, playhead/scissors/trash positioning, segment hover/selection, division handle creation/drag setup, append button positioning. Animation orchestration (`animateSegmentDelete`/`animateSegmentRestore`/`captureSegmentBitmap`) calls into `segment-anim.js` for canvas-level rendering. This is the largest and most visual-detail-heavy module.
