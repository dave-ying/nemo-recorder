...
- **`waveform.js`** — canvas rendering for the playback waveform: segment "cards" with gaps, peak computation from the recorded buffer, playhead/scissors/trash positioning, segment hover/selection, division handle creation/drag setup, append button positioning. Animation orchestration (`animateSegmentDelete`/`animateSegmentRestore`/`captureSegmentBitmap`) calls into `segment-anim.js` for canvas-level rendering. Also owns the segment-reorder drag animation rAF loop (`ensureDragAnimRunning`/`drawDragFrame`): while active, it renders the live arrangement (non-dragged segments easing toward would-be positions, a floating dragged card following the pointer with lift + deep shadow, a dashed drop-zone outline) and a post-release settle phase; `drawPlaybackWaveform` bails out while this loop is running so the drag frame is never clobbered. This is the largest and most visual-detail-heavy module.
