...
- **`state.js`** — the single `AppState` object (JSDoc-typed) plus layout/style constants (`WAVEFORM_STYLE` colors, segment gap/corner/shadow px, `LIVE_SECONDS`, `MIN_SEGMENT_SAMPLES`). Key field groups: recording-related (`audioContext`, `mediaStream`, `recordedChunks`, `isRecording`), playback (`recordedBuffer`, `playbackOffset`, `isPlaying`), segment editing (`originalBuffer`, `segments`, `selectedSegmentIndex`, `hoverSegmentIndex`), live preview (`pendingTakeBuffer`, `previewOffset`, `isPreviewing`, `recordModalContext`), drag state (`draggingHandleIndex`, `_dragSnapshot` typed as `DragSnapshot|null`, `draggingPlayhead`, `draggingSegmentIndex`, `pendingSegmentDrag`, `_segmentDragSnapshot` typed as `SegmentDragSnapshot|null`), and history-independents (`exportSettings`, `settings`, `micCapabilities`, `micLabel`). Start here to understand what fields exist before touching any feature.
...
- **`waveform.js`** — canvas rendering for the playback waveform: segment "cards" with gaps, peak computation from the recorded buffer, playhead/scissors positioning, segment hover/selection (including the selection/delete pulse animation driven by `state.isHoveringTrash`, now triggered by hovering the toolbar delete button rather than an on-canvas control), division handle creation/drag setup, append button positioning. Animation orchestration (`animateSegmentDelete`/`animateSegmentRestore`/`captureSegmentBitmap`) calls into `segment-anim.js` for canvas-level rendering. Also owns the segment-reorder drag animation rAF loop (`ensureDragAnimRunning`/`drawDragFrame`): while active, it renders the live arrangement (non-dragged segments easing toward would-be positions, a floating dragged card following the pointer with lift + deep shadow, a dashed drop-zone outline) and a post-release settle phase; `drawPlaybackWaveform` bails out while this loop is running so the drag frame is never clobbered. This is the largest and most visual-detail-heavy module.
