...
- **`state.js`** — the single `AppState` object (JSDoc-typed) plus layout/style constants (`WAVEFORM_STYLE` colors, segment gap/corner/shadow px, `LIVE_SECONDS`). Key field groups: recording-related (`audioContext`, `mediaStream`, `recordedChunks`, `isRecording`), playback (`recordedBuffer`, `playbackOffset`, `isPlaying`), segment editing (`originalBuffer`, `segments`, `selectedSegmentIndex`, `hoverSegmentIndex`), live preview (`pendingTakeBuffer`, `previewOffset`, `isPreviewing`, `recordModalContext`), drag state (`draggingPlayhead`, `draggingSegmentIndex`, `pendingSegmentDrag`, `_segmentDragSnapshot` typed as `SegmentDragSnapshot|null`), and history-independents (`exportSettings`, `settings`, `micCapabilities`, `micLabel`). Start here to understand what fields exist before touching any feature.
